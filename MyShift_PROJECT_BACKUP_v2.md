...
> Generated: 2026-05-12 (updated)
> Version: 2.0.0 (versionCode 11)
> Developer: Any WeCon (fiouri.work@gmail.com)
> Package: com.anywecon.myshiftapp (final Play Store applicationId)
> GitHub: https://github.com/Fiouri/myshift
> Privacy Policy: https://fiouri.github.io/myshift/privacy-policy.html (LIVE)
> Play Store: Submitted for review — Closed Testing track
...
- **GitHub:** https://github.com/Fiouri/myshift
- **Phase:** Phase 7 — Branding refresh & Play Store submission (submitted for review, closed testing track)
...
│
├── assets/
│   ├── adaptive-icon.png              — Android adaptive icon (1024×1024, transparent bg, "My"+"Shift" in safe zone)
│   ├── favicon.png                    — Web favicon
│   ├── icon.png                       — App icon (1024×1024, "My" white 190px / "Shift" #90CAF9 160px on #1565C0→#0A2E6B gradient, rx=108)
│   └── splash-icon.png               — Splash screen icon
│
├── docs/
│   ├── index.html                     — GitHub Pages landing (Any WeCon footer, fiouri.work@gmail.com contact)
│   ├── privacy-policy.html           — Bilingual privacy policy (EL/EN, GDPR)
│   ├── play-store-icon-512.png       — 512×512 Play Store icon (resized from icon.png)
│   ├── feature-graphic.png           — 1024×500 Play Store feature graphic
│   ├── play-store-listing.md         — Play Store metadata (EL+EN)
│   └── screenshot-guide.md           — Screenshot capture instructions
│
├── scripts/
│   └── generate-icons.js             — Sharp-based SVG→PNG renderer for icon assets (re-runnable)
│
├── __tests__/
...
- **Package:** `com.anywecon.myshiftapp` *(FINAL Play Store applicationId; v1 slug kept for EAS continuity)*

### Keystore (Play Store signing)
- **Status:** ✅ NEW — regenerated by EAS for `com.anywecon.myshiftapp`
- **Source:** EAS-generated (auto-managed key alias, hex format)
- **Local references:** `credentials.json` → `credentials/android/keystore.jks` (the `.jks` file itself is gitignored)
- **Managed via:** `eas credentials` (recommended) — locally stored copy for offline signing
- **⚠️ Critical:** This keystore is now bound to the Play Store app. Do NOT regenerate or lose it — would brick all future updates.

### Privacy Policy
- **URL:** `https://fiouri.github.io/myshift/privacy-policy.html` *(LIVE)*
- **Contact Email:** `fiouri.work@gmail.com` *(ΟΧΙ info@anywecon.com, ΟΧΙ support@fiouri.com — fiouri.work@gmail.com is the canonical email)*
...
- [x] **Play Store AAB Build** — versionCode 11 production AAB ✅
...
- [x] **Keystore Configured** — New EAS-generated keystore for com.anywecon.myshiftapp ✅
- [x] **App Icon Refresh** — New "My"+"Shift" stacked design on blue gradient ✅
- [x] **Feature Graphic** — 1024×500 PNG ready (`docs/feature-graphic.png`) ✅
- [x] **Play Store icon (512×512)** — Generated (`docs/play-store-icon-512.png`) ✅
- [x] **Play Store Submission** — Submitted for review on Closed Testing track ✅
...
| v2.0.0 | 7–10 | 2026-04 → 2026-05 | **Phase 6.5 — Closed testing iteration builds.** EAS auto-incremented versionCodes for closed-testing AABs uploaded to Play Console (autoIncrement enabled in eas.json production profile). |
| v2.0.0 | 11 | 2026-05-12 | **Phase 7 — Branding & submission.** New app icon design ("My" white 190px + "Shift" #90CAF9 160px on #1565C0→#0A2E6B dark blue gradient, rx=108). Adaptive icon with transparent bg in safe zone. 512×512 Play Store icon + 1024×500 feature graphic generated via `scripts/generate-icons.js` (sharp + SVG). New EAS-generated keystore for `com.anywecon.myshiftapp`. Code-level email/developer canonicalization: `appConfig.ts` DEVELOPER→"Any WeCon", SUPPORT_EMAIL→"fiouri.work@gmail.com"; `docs/index.html` mailto + footer aligned. AAB versionCode 11 submitted to Closed Testing track. |
...
| production | AAB | 6 | ✅ | Initial production build (Phase 6 polish) |
| production | AAB | 7–10 | ✅ | Closed-testing iteration builds (EAS autoIncrement) |
| **production** | **AAB** | **11** | **✅ Submitted to Closed Testing** | New keystore + new icon/feature graphic + canonical email/developer fields |
...
- **Keystore:** NEW EAS-generated keystore for com.anywecon.myshiftapp ✅
- **Developer:** Any WeCon ✅
- **Contact email:** fiouri.work@gmail.com ✅

### Immediate — DONE ✅
1. ~~Run `npx tsc --noEmit`~~ ✅
2. ~~Run `npx jest` (106 tests)~~ ✅
3. ~~`eas build --profile preview`~~ ✅
4. ~~`eas build --profile production` (versionCode 11)~~ ✅
5. ~~Play Store listing~~ ✅
6. ~~Privacy Policy published~~ ✅
7. ~~Generate keystore via EAS for com.anywecon.myshiftapp~~ ✅
8. ~~Generate app icon + adaptive icon + 512×512 icon~~ ✅
9. ~~Generate feature graphic (1024×500)~~ ✅
10. ~~Take screenshots~~ ✅
11. ~~Upload AAB + assets to Play Console~~ ✅
12. ~~Submit for review (Closed Testing)~~ ✅

### Awaiting
- ⏳ Google Play review verdict (Closed Testing) → promotion to Production track
- ⏳ Tester feedback from Closed Testing cohort
...
> **Note:** This backup reflects the codebase as of **2026-05-12** (versionCode 11). All values in this document have been verified against the live codebase (`app.json`, `eas.json`, `package.json`, `credentials.json`, `src/constants/appConfig.ts`, `src/i18n/{el,en}.json`, `docs/privacy-policy.html`, `docs/play-store-listing.md`, `docs/index.html`). Phase 7 brought a branding refresh: new icon design ("My" + "Shift" stacked, dark blue gradient) + 1024×500 feature graphic + 512×512 Play Store icon, all generated via `scripts/generate-icons.js`. New EAS-generated keystore for `com.anywecon.myshiftapp`. Email canonicalized to `fiouri.work@gmail.com` and developer to `Any WeCon` across `appConfig.ts` and `docs/index.html` (was `support@fiouri.com` / `Fiouri`). AAB versionCode 11 submitted for review on Closed Testing track.
